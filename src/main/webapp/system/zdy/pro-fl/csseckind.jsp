...
strSQL = "select id,proflname,snumber from secprofl where firstkind='"+firstkind+"'  order by snumber asc";
sqlRst = einfodb.executeQuery(strSQL);
...
<font face="Arial, Helvetica, sans-serif" size="2">---&gt;:<a href="cszjkindf.jsp" style="font-size: 12px"><%=proname1%></a></font><br>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
  ...
  <tr height="8"> 
    ...
    String t=snumber.substring(2,snumber.length());
	  %>
    <td width="100%" height="2"> 
      <p><b>＊　</b><font size="2" face="Arial, Helvetica, sans-serif"><a href="czthkindf.jsp?proname=<%=proname%>&seckind=<%=sid%>&id<%=firstkind%>&profl=<%=proname1%>" target="right" style="font-size: 12px"><%=t%></a> 
        <a href="czthkindf.jsp?proname=<%=proname%>&seckind=<%=sid%>&id<%=firstkind%>&profl=<%=proname1%>" target="right" style="font-size: 12px"><%=proname%></a></font></p>
    </td>
  </tr>
  <% i++; } %>
  <tr> 
    <td align=center height="22"> 
      <div align="right"><font color="#000000" size="2" face="Arial, Helvetica, sans-serif"> 
        ...
        <a href="csseckind.jsp?page=<%=intPage+1%>&firstkind=<%=firstkind%>&proname=<%=proname1%>" style="font-size: 12px">下一页</a> 
        <% 
}

%>
        <%if(intPage>1){%>
        <a href="csseckind.jsp?page=<%=intPage-1%>&firstkind=<%=firstkind%>&proname=<%=proname1%>" style="font-size: 12px">上一页</a> 
        <% 
}
