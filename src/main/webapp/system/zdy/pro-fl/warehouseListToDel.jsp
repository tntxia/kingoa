...
strSQL = "select count(*) from warehouse  where pro_addr = '"+addr+"' and pro_num = 0 and id not in (select oid from o_pic)";
...
String  strSQLup = "select price_hb,sum(pro_num*pro_price) as zprice from warehouse  pro_addr = '"+addr+"' and pro_num = 0 and id not in (select oid from o_pic)";
...
strSQL = "select top "+top+" id,pro_number,pro_model,pro_gg,pro_name,pro_sup_number,yqdate,pro_secid,pro_supplier,pro_num,pro_unit,pro_min_num,pro_price,price_hb,saleprice,pro_type,pro_yyfw,pro_addr,pro_remark,th_number from warehouse where  pro_addr = '"+addr+"' and pro_num = 0 and id not in (select oid from o_pic) order by pro_model";
if(actionType!=null && actionType.equals("query"))
{
strSQL = "select id,pro_number,pro_model,pro_gg,pro_name,pro_sup_number,yqdate,pro_secid,pro_supplier,pro_num,pro_unit,pro_min_num,pro_price,price_hb,saleprice,pro_yyfw,pro_addr,pro_remark,th_number from warehouse where  pro_addr = '"+addr+"' and pro_num=0 and id not in (select oid from o_pic) order by pro_model";

strSQLup = "select  price_hb,sum(pro_num*pro_price)  as zprice  from warehouse where pro_addr = '"+addr+"'";
}
...
%>
  ...
  <%if(intPage>1){%>
  <a href="warehouseList.jsp?page=<%=intPage-1%>&addr=<%=addr%>">上一页</a> 
  <% } %>
  <%if(intPage<intPageCount){%>
  <a href="warehouseList.jsp?page=<%=intPage+1%>&addr=<%=addr%>">下一页</a> 
  <%}%>
